"""PymolFold_min: for API-driven Structure Prediction and Quality Assessment.
...
def ensure_package(pkg_name):
    ...
    if should_install:
        try:
            ...
            print("https://github.com/JinyuanSun/PymolFold_min/blob/main/README.md")
            print("---------------------------------------------------------------")
        except subprocess.CalledProcessError as e:
            print(f"ERROR: Failed to install {pkg_name}. details: {e}")


# Ensure pymolfold_min is installed or upgraded to latest
ensure_package("pymolfold_min")

import pymolfold_min
...
# Initialize plugin
pymolfold_min.plugin.__init_plugin__()


class PymolFold_minDialog(QtWidgets.QDialog):
    """
    Qt-based GUI front-end for the PymolFold_min command-line plugin.
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("PymolFold_min – Structure Prediction")
        self.resize(700, 500)
        ...
        try:
            self.workdir_edit.setText(pymolfold_min.plugin.ABS_PATH)
        except NameError:
            pass
        ...
        def worker():
            try:
                if predictor == "ESM-3":
                    ...
                    pymolfold_min.plugin.query_esm3(
                        sequence=seq,
                        name=name,
                    ...
                    pymolfold_min.plugin.query_esmfold(sequence=seq, name=name)
                else:
                    self.append_log("Calling query_boltz_monomer(...)")
                    pymolfold_min.plugin.query_boltz_monomer(sequence=seq, name=name)
    ...
    def on_fetch_af(self):
        ...
        def worker():
            try:
                pymolfold_min.plugin.fetch_af(uniprot_id)
    ...
    def on_fetch_am(self):
        ...
        def worker():
            try:
                pymolfold_min.plugin.query_am_hegelab(name)
    ...
    def on_set_workdir(self):
        ...
        if not path:
            ...
            return
        try:
            pymolfold_min.plugin.set_workdir(path)
    ...
    def on_set_api_key(self):
        ...
        try:
            pymolfold_min.plugin.set_api_key(key_name, key_value)
    ...
    def on_open_webui(self):
        ...
        def worker():
            try:
                pymolfold_min.plugin.init_boltz2_gui()
...
def show_pymolfold_min_dialog():
    """
    Create or raise the Qt dialog from the PyMOL plugin menu.
    """
    from pymol import plugins

    # from pymolfold_min.plugin import PymolFold_minDialog

    global _pymolfold_min_dialog

    # Parent to PyMOL main window if available
    parent = plugins.get_qtwindow() if hasattr(plugins, "get_qtwindow") else None

    # if _pymolfold_min_dialog is None or not _pymolfold_min_dialog.isVisible():
    _pymolfold_min_dialog = PymolFold_minDialog(parent=parent)
    _pymolfold_min_dialog.show()
    _pymolfold_min_dialog.raise_()
    _pymolfold_min_dialog.activateWindow()


def __init_plugin__(app=None):
    """Initialize the PymolFold_min plugin in PyMOL."""
    from pymol.plugins import addmenuitemqt

    addmenuitemqt("PymolFold_min Streamlit GUI", pymolfold_min.plugin.init_boltz2_gui)
    addmenuitemqt("PymolFold_min PyQt GUI", show_pymolfold_min_dialog)
